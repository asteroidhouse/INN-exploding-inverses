...
class FlowStep(nn.Module):
    def __init__(self, in_channels, hidden_channels, actnorm_scale,
        ...
        if flow_permutation == "invconv":
            ...
            self.flow_permutation = lambda z, zn, logdet, rev: self.invconv(z, zn, logdet, rev)
        elif flow_permutation == "shuffle":
            self.shuffle = Permute2d(in_channels, shuffle=True)
            self.flow_permutation = lambda z, zn, logdet, rev: (self.shuffle(z, zn, rev), logdet)
        else:
            self.reverse = Permute2d(in_channels, shuffle=False)
            self.flow_permutation = lambda z, zn, logdet, rev: (self.reverse(z, zn, rev), logdet)
    ...
    def forward(self, input, zn, logdet=None, reverse=False):
        if not reverse:
            return self.normal_flow(input, zn, logdet)
        else:
            return self.reverse_flow(input, zn, logdet)

    def normal_flow(self, input, zn, logdet):
        assert input.size(1) % 2 == 0

        # 1. actnorm
        z, logdet, zn = self.actnorm(input, zn, logdet=logdet, reverse=False)

        # 2. permute
        z, logdet, zn = self.flow_permutation(z, zn, logdet, False)
    ...
    def reverse_flow(self, input, zn, logdet):
        assert input.size(1) % 2 == 0
        ...
        # 2. permute
        z, logdet, zn = self.flow_permutation(z, zn, logdet, True)

        # 3. actnorm
        z, logdet, zn = self.actnorm(z, zn, logdet=logdet, reverse=True)
...
class FlowNet(nn.Module):
    ...
    def forward(self, input, zn, logdet=0., reverse=False, temperature=None):
        if reverse:
            return self.decode(input, zn, temperature)
        else:
            return self.encode(input, zn, logdet)

    def encode(self, z, zn, logdet=0.0):
        for layer, shape in zip(self.layers, self.output_shapes):
            z, logdet, zn = layer(z, zn, logdet, reverse=False)
        return z, logdet, zn

    def decode(self, z, zn, temperature=None):
        for layer in reversed(self.layers):
            if isinstance(layer, Split2d):
                z, logdet, zn = layer(z, zn, logdet=0, reverse=True, temperature=temperature)
            else:
                z, logdet, zn = layer(z, zn, logdet=0, reverse=True)
        return z
...
class Glow(nn.Module):
    ...
    def forward(self, x=None, y_onehot=None, z=None, zn=None, temperature=None,
                reverse=False):
        if reverse:
            return self.reverse_flow(z, zn, y_onehot, temperature)
        else:
            return self.normal_flow(x, zn, y_onehot)
