...
class ResidualFlow(nn.Module):
    ...
    def inverse(self, z, logpz=None, num_iter=None):
        if self.factor_out:
            ...
            if logpz is None:
                if isinstance(self.transforms[-1], StackediResBlocks):
                    z_prev = self.transforms[-1].inverse(zs[-1], num_iter=num_iter)
                else:
                    z_prev = self.transforms[-1].inverse(zs[-1])
                for idx in range(len(self.transforms) - 2, -1, -1):
                    z_prev = torch.cat((z_prev, zs[idx]), dim=1)
                    if isinstance(self.transforms[idx], StackediResBlocks):                
                        z_prev = self.transforms[idx].inverse(z_prev, num_iter=num_iter)
                    else:
                        z_prev = self.transforms[idx].inverse(z_prev)
                return z_prev
            else:
                if isinstance(self.transforms[-1], StackediResBlocks):
                    z_prev, logpz = self.transforms[-1].inverse(zs[-1], logpz, num_iter=num_iter)
                else:
                    z_prev, logpz = self.transforms[-1].inverse(zs[-1], logpz)
                for idx in range(len(self.transforms) - 2, -1, -1):
                    z_prev = torch.cat((z_prev, zs[idx]), dim=1)
                    if isinstance(self.transforms[idx], StackediResBlocks):
                        z_prev, logpz = self.transforms[idx].inverse(z_prev, logpz, num_iter=num_iter)
                    else:
            ...
            for idx in range(len(self.transforms) - 1, -1, -1):
                if logpz is None:
                    if isinstance(self.transforms[idx], StackediResBlocks):
                        z = self.transforms[idx].inverse(z, num_iter=num_iter)
                    else:
                        z = self.transforms[idx].inverse(z)
                else:
                    if isinstance(self.transforms[idx], StackediResBlocks):
                        z, logpz = self.transforms[idx].inverse(z, logpz, num_iter=num_iter)
                    else:
