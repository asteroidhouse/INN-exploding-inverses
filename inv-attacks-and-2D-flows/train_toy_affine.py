...
parser.add_argument('--arch', choices=['iresnet', 'realnvp'], default='realnvp')
...
parser.add_argument('--save', type=str, default='experiments/realnvp_toy_sigmoid05_100blocks')
...
def build_lipschitz_nnet(dims, activation_fn=torch.nn.ReLU):
    nnet = []
...
if __name__ == '__main__':
    ...
    if args.arch == 'iresnet':
        ...
        for _ in range(args.nblocks):
            blocks.append(
                layers.iResBlock(
                    build_lipschitz_nnet(dims, activation_fn),
                    n_dist=args.n_dist,
    ...
    # save model
    torch.save(model, os.path.join(args.save, 'savedModel'))

    # test model
    p_trial_samples = toy_data.inf_train_gen(args.data, batch_size=2)
